...
> Zweck: Ist-Zustand-Dokumentation des Projekts (v1.0.46). Synchronisiert mit BERICHT.md (§1–§21) und den Builds in `Ausgabe\`.
...
├── xyt-downloader.user.js                  ← ARBEITSVERSION (v1.0.46, 1922 Zeilen, ~87,8 KB) — projekteigener Code
├── BERICHT.md                              ← Abschlussbericht/Projektdoku (v1.0.0–v1.0.46, §1–§21)
├── AGENTS.md                               ← Projekt-Anweisungen für Agenten (Build-Regeln, Architektur, Tests)
├── DOKUMENTATION_ENTWICKLUNGSSTAND.md      ← diese Datei (Ist-Zustand v1.0.46)
...
├── Ausgabe\                                ← Build-Ordner (versionierte Builds v1.0.0 … v1.0.46, alte bleiben erhalten)
...
│   └── xyt-downloader-v1.0.46.user.js      ← AKTUELLER BUILD (identisch mit Arbeitsversion, cmp-verifiziert,
│                                             MD5 de2a92e1b9110c70c46fad155856064d)
...
**Codestellen (v1.0.46):** `getVisitorData()` (~Z. 771), `fetchAndroidVrPlayer()` (~Z. 788), `extractStreams()` (~Z. 933), `downloadUrl()` (~Z. 319), `mergeFmp4()` (~Z. 591), `runDownload()` (~Z. 1603).
...
- **Position (v1.0.43):** kleines, dezentes Overlay **unten rechts im Player** (`.xyt-dl-overlay`, 74×23 px, opacity 0.85, z-index 9999); auf /watch bevorzugt in der Action-Leiste `#top-level-buttons-computed`.
...
## 4. Fehleranalyse: Chronologie der behobenen Ursachen (v1.0.0 → v1.0.46)
...
Aktuelle Real-Tests (Playwright + echter ANDROID_VR-Request) je Version: siehe **BERICHT.md** pro Abschnitt. Zusammengefasst für v1.0.40–v1.0.46:
...
## 6. Veröffentlichung (v1.0.46)

| Ziel | URL | Status |
|---|---|---|
| Greasy Fork | https://greasyfork.org/de/scripts/589972-xytdownloader | ✅ veröffentlicht (v1.0.46, Konto `immerzu`) |
...
- **Volltext der Arbeitsversion:** siehe `xyt-downloader.user.js` (1922 Zeilen, ~87,8 KB) und identisch `Ausgabe\xyt-downloader-v1.0.46.user.js` (cmp-verifiziert, MD5 `de2a92e1b9110c70c46fad155856064d`).
- **BERICHT.md:** vollständige Projektdoku (Erfolgskriterien, Loop, Fixes v1.0.0–v1.0.46, §1–§21).
